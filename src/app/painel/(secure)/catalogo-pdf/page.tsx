...
export default async function CatalogPdfPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const availability = params.disponibilidade === "estoque" ? "estoque" : "todos";
  const products = allProducts.filter((product) => product.status === "active" && (!category || product.category_id === category) && (!onlyOffers || product.is_offer) && (availability !== "estoque" || product.stock > 0));
  return <>
    <div className="admin-no-print"><AdminPageHeader eyebrow="Divulgacao" title="Catalogo PDF" description="Escolha todos os itens ou somente os que estão em estoque e salve o PDF." actions={<PrintButton />} /><PanelCard className="mb-7"><form className="grid gap-3 sm:grid-cols-[1fr_180px_auto_auto_auto] sm:items-end"><label className="text-xs font-bold">Categoria<select name="categoria" defaultValue={category} className="mt-1.5 w-full rounded-lg border border-ink-200 px-3 py-2.5 text-sm"><option value="">Todas</option>{categories.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-xs font-bold">Itens<select name="disponibilidade" defaultValue={availability} className="mt-1.5 w-full rounded-lg border border-ink-200 px-3 py-2.5 text-sm"><option value="todos">Todos os itens</option><option value="estoque">Somente em estoque</option></select></label><label className="pb-2 text-sm font-semibold"><input type="checkbox" name="ofertas" value="1" defaultChecked={onlyOffers} className="mr-2 accent-gold-500" />Somente ofertas</label><label className="pb-2 text-sm font-semibold"><input type="checkbox" name="estoque" value="1" defaultChecked={showStock} className="mr-2 accent-gold-500" />Mostrar estoque</label><button className="rounded-lg bg-ink-900 px-4 py-2.5 text-sm font-bold text-white">Atualizar</button></form></PanelCard></div>
