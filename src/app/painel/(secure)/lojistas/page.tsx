...
import { formatarDesconto, itensSemPrecoParaLojistas, linhasParaLojistas } from "@/lib/admin/lojistas";
  ...
  return <>
    <div className="admin-no-print space-y-6">
      ...
      <PanelCard>
        ...
        <p className="mb-4 mt-1 text-sm text-ink-500">Para baixar mais um item só: fica no lugar do preço com o desconto geral, e precisa ser menor que ele. Vazio volta ao desconto geral. Entram todos os produtos publicados, com ou sem estoque (uma linha por opção nos que têm variação).</p>
        {semPreco.length > 0 && (
          <p role="status" className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
            {semPreco.length === 1 ? "1 item publicado ficou fora" : `${semPreco.length} itens publicados ficaram fora`} por não ter preço à vista no cadastro: <span data-dado-do-dono>{semPreco.join("; ")}</span>.
          </p>
        )}
