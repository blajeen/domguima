...
export default async function InventoryPage() {
  ...
  return <>
    ...
    <PanelCard className="mt-5"><h2 className="text-base font-black">Historico de movimentacoes</h2><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs uppercase text-ink-400"><tr><th className="py-2">Data</th><th>Produto</th><th>Movimento</th><th>Antes → Depois</th><th>Motivo</th><th>Observacao</th></tr></thead><tbody className="divide-y divide-ink-100">{movements.map((movement) => <tr key={String(movement.id)}><td className="py-3 text-xs text-ink-500">{new Date(String(movement.created_at)).toLocaleString("pt-BR")}</td><td className="font-semibold">{(movement.products as { name?: string } | null)?.name ?? movement.product_id}</td><td className={`font-black ${Number(movement.quantity_delta) > 0 ? "text-success" : "text-red-600"}`}>{Number(movement.quantity_delta) > 0 ? "+" : ""}{movement.quantity_delta}</td><td>{movement.stock_before} → {movement.stock_after}</td><td>{reasonLabel(String(movement.reason))}</td><td className="text-ink-500">{movement.note || "—"}</td></tr>)}</tbody></table>{movements.length === 0 && <p className="py-8 text-center text-sm text-ink-500">Nenhuma movimentacao registrada ainda.</p>}</div></PanelCard>
  </>;
}
