          ...
          <Card title="Entrega e pagamento" step={3}>
            ...
            <fieldset className="mt-4" data-field="paymentMethod">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {paymentOptions(isUberlandia(form.city), taxas).map((option) => (
                  <label
                    key={option.value}
              ...
              {/* "quando o cliente vai fechar sobe essa tabela calculada ja caso
                  seja parcelado" (dono): escolheu crédito, as parcelas do total
                  aparecem abertas. O frete entra depois, na confirmação. */}
              {form.paymentMethod === "credit_card" && (
                <TabelaDeParcelas
                  cents={subtotal}
                  aberta
                  rotulo="Parcelas no cartão (sem o frete)"
                  className="mt-3"
                />
              )}
...
function paymentOptions(isLocal: boolean, taxas: TaxasDoCartao): Array<{ value: OrderPaymentMethod; label: string; description: string }> {
  const options: Array<{ value: OrderPaymentMethod; label: string; description: string }> = [
    ...
    { value: "credit_card", label: ORDER_PAYMENT_METHOD_LABELS.credit_card, description: descricaoDoCredito(taxas) },
...
/** A opção de crédito diz até quantas vezes a tabela vai, e se a última tem taxa. */
function descricaoDoCredito(taxas: TaxasDoCartao): string {
  if (taxas.length < 2) return "Combinamos a cobrança com você.";
  const ultima = taxas[taxas.length - 1];
  return ultima === 0
    ? `Em até ${taxas.length}x sem juros.`
    : `Em até ${taxas.length}x, com a taxa da maquininha.`;
}
