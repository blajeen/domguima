  ...
  return <div className="inventory-sheet">
    <div className="inventory-sheet-head">
      ...
      <div className="inventory-mode-switch" aria-label="Modo da planilha">
        ...
        <button type="button" className={mode === "sales" ? "is-active" : ""} onClick={() => setMode("sales")} aria-pressed={mode === "sales"}>Saídas do dia {pendingExits > 0 && <b>{pendingExits}</b>}</button>
      </div>
    </div>
    ...
    {mode === "sales" && hasUnsavedCount && <div className="inventory-context-note">Salve primeiro as contagens pendentes para registrar saídas com a quantidade mais atual.</div>}
    ...
    {mode === "sales" && pendingExits > 0 && <div className="inventory-savebar sales"><div><strong>{pendingExits} {pendingExits === 1 ? "unidade para baixar" : "unidades para baixar"}</strong><span>{hasUnsavedCount ? "Salve as contagens antes de confirmar." : "A operação ficará registrada no histórico como venda."}</span></div><div className="inventory-save-actions"><button type="button" className="inventory-secondary-action" onClick={() => { setDrafts((current) => Object.fromEntries(Object.entries(current).map(([id, draft]) => [id, { ...draft, exitQty: 0 }]))); salesBatchId.current = null; }} disabled={isPending}>Limpar saídas</button><button type="button" className="inventory-primary-action sales" onClick={registerSales} disabled={isPending || hasUnsavedCount}>{isPending ? "Registrando..." : "Registrar saídas"}</button></div></div>}

    <div className="inventory-guide"><div className="inventory-guide-icon">i</div><div><strong>Fluxo recomendado</strong><p>1. Faça a contagem física e salve. 2. No fim do dia, abra “Saídas do dia”, informe as quantidades e confirme uma única vez.</p></div><span className="inventory-shortcut"><kbd>Tab</kbd> navega entre células <kbd>+</kbd> / <kbd>−</kbd> ajustam 1 unidade</span></div>
