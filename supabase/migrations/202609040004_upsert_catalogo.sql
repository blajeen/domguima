-- ============================================================================
-- replace_catalog_state SEM APAGAR OS PRODUTOS
-- ============================================================================
--
-- Problema que esta migracao resolve, medido em producao:
--
--   inventory_movements.product_id references public.products(id) ON DELETE CASCADE
--
-- A funcao comecava com `delete from public.products where true`, e o CASCADE
-- levava junto TODO o historico de movimentacao de estoque. Salvar um produto
-- qualquer no painel apagava os movimentos de todos os 95. Tirar o `delete
-- from public.inventory_movements` explicito (202609040002) nao bastou: quem
-- destruia era o cascade. Os audit_logs sobreviviam porque nao tem essa FK.
--
-- A correcao: em vez de apagar tudo e reinserir, fazemos upsert dos produtos e
-- categorias e removemos apenas os que sairam do catalogo. Produto que
-- permanece nunca e apagado, entao o cascade nunca dispara e o historico fica.
--
-- Guarda de seguranca: se o payload vier com a lista de produtos ou categorias
-- vazia, NADA e removido. Antes, um payload vazio zerava o catalogo inteiro.
--
-- Rollback: reaplicar a definicao de 202609040002_ledger_cutover.sql.
-- ============================================================================
...
begin
  ...
  --    mantem a ordenacao exatamente como o painel montou.
